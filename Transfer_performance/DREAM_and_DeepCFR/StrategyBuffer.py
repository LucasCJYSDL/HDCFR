...
class StrategyBuffer:
    ...
    def add(self, iteration_strat):
        if self._max_size is None or (self._size < self._max_size):
            ...
            self._weights.append(iteration_strat.iteration + 1) # this weight may be used for t-CFR

            self._size = len(self._strategies)

        elif np.random.random() < (float(self._max_size) / float(self._last_iteration_seen)):
