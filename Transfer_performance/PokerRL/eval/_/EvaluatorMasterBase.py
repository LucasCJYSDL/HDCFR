...
class EvaluatorMasterBase(WorkerBase):
    ...
    def _log_results(self, agent_mode, stack_size_idx, iter_nr, score, upper_conf95=None, lower_conf95=None):
        ...
        if self._log_conf_interval:
            ...
            self._ray.remote(self._chief_handle.add_scalar,
                             self._exp_names_conf[agent_mode][stack_size_idx][0], graph_name+'low_conf', iter_nr, lower_conf95)
            self._ray.remote(self._chief_handle.add_scalar,
                             self._exp_names_conf[agent_mode][stack_size_idx][1], graph_name+'high_conf', iter_nr, upper_conf95)
    ...
    def _log_multi_stack(self, agent_mode, iter_nr, score_total, upper_conf95=None, lower_conf95=None):
        ...
        if self._log_conf_interval:
            ...
            assert lower_conf95 is not None
            self._ray.remote(self._chief_handle.add_scalar,
                             self._exp_names_multi_stack_conf[agent_mode][0], graph_name+'low_conf', iter_nr, lower_conf95)
            self._ray.remote(self._chief_handle.add_scalar,
                             self._exp_names_multi_stack_conf[agent_mode][1], graph_name+'high_conf', iter_nr, upper_conf95)
