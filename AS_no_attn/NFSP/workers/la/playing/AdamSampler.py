...
class AdamSampler(SamplingAlgo):

    def __init__(self, t_prof, env_bldr, br_buf2, avg_buf2, br_learner2, avg_learner2, constant_eps):
        super().__init__(t_prof=t_prof, env_bldr=env_bldr, br_buf2=br_buf2, avg_buf2=avg_buf2,
                         ...
                         n_envs_br=t_prof.n_steps_br_per_iter_per_la, n_envs_avg=t_prof.n_steps_avg_per_iter_per_la)
        ...
        self._seat_actors = [
            _SeatActor(t_prof=t_prof, env_bldr=self._env_bldr, seat_id=p,
                       ...
                       br_learner=br_learner2[p], avg_learner=avg_learner2[p], constant_eps=constant_eps, sampler=self)
            for p in range(self._env_bldr.N_SEATS)
        ]
    ...
    def play(self, nfsp_iter):
        ...
        self.states_traversed += self._t_prof.n_steps_br_per_iter_per_la + self._t_prof.n_steps_avg_per_iter_per_la
...
class _SeatActor:
    ...
    def __init__(self, seat_id, t_prof, env_bldr, br_memory_savers, avg_buf_savers, br_learner, avg_learner,
                 constant_eps, sampler):
        self.owner = seat_id

        self._t_prof = t_prof
        self._env_bldr = env_bldr

        self._constant_eps = constant_eps
        ...
        self._current_policy_tags_OPP_BR = None
    ...
    def init(self, sws_br, sws_avg, nfsp_iter):
        ...
        self._current_policy_tags_OPP_BR = np.empty(shape=self._t_prof.n_steps_br_per_iter_per_la, dtype=np.int32)
        for sw in sws_br:
            self._current_policy_tags_OPP_BR[sw.env_idx] = SeatActorBase.pick_training_policy(
    ...
    def act_for_br_opp(self, step_wrappers):
        ...
        SeatActorBase.act_mixed(step_wrappers=step_wrappers, br_learner=self.br_learner, owner=self.owner,
                                avg_learner=self.avg_learner, current_policy_tags=self._current_policy_tags_OPP_BR,
                                random_prob=0)
    ...
    def act_for_avg_opp(self, step_wrappers):
        ...
        """
        SeatActorBase.act_avg(step_wrappers=step_wrappers, owner=self.owner, avg_learner=self.avg_learner, )
