...
class StrategyFiller:
    ...
    def fill_uniform_random(self, p_id=None):
        ...
        self._fill_uniform_random(node=self._tree.root, p_id=p_id)
    ...
    def fill_with_agent_policy(self, agent, is_hier):

        self.stat_dict = [0, 0]

        print("Init: ", self.stat_dict)
        ...
        if not is_hier:
            ...
            self._fill_with_agent_policy(node=self._tree.root, agent=agent, is_hier=is_hier, last_option=[None, None])

        self.update_reach_probs()

        print("Important: ", self.stat_dict)

        return float(self.stat_dict[0]) / float(self.stat_dict[1])

    def update_reach_probs(self):
        self._update_reach_probs(node=self._tree.root)

    def _fill_uniform_random(self, node, p_id):
        ...
        if (p_id is None) or (node.p_id_acting_next == p_id):
        ...
        for c in node.children:
            self._fill_uniform_random(node=c, p_id=p_id)
    ...
    def _fill_with_agent_policy(self, node, agent, is_hier=False, last_option=[None, None]):
        """
        ...
        self.stat_dict[1] += self._env_bldr.rules.RANGE_SIZE
        ...
        if isinstance(node, ChanceNode) or (isinstance(node, PlayerActionNode)
            ...
            if not is_hier:
                ...
                agent_strat, cur_option_id = agent.get_a_probs_for_each_hand(last_option)
                # print(cur_option[node.p_id_acting_next])
                if cur_option[node.p_id_acting_next] is not None:
                    t_last_options = cur_option[node.p_id_acting_next].cpu().numpy()
                    t_cur_options = cur_option_id.cpu().numpy()
                    n_non_match = 0
                    for i in range(len(t_cur_options)):
                        if t_cur_options[i] != t_last_options[i]:
                            n_non_match += 1
                    self.stat_dict[0] += n_non_match

                cur_option[node.p_id_acting_next] = cur_option_id
        ...
        for c in node.children:
            if not is_hier:
                ...
                self._fill_with_agent_policy(node=c, agent=agent, last_option=cur_option, is_hier=is_hier)
