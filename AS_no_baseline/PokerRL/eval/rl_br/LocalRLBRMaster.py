...
class LocalRLBRMaster(EvaluatorMasterBase):
    ...
    def _retrain(self, br_number, mode, stack_size, stack_size_idx, global_iter_nr):
        ...
        print("Training RL-BR with agent mode", mode, "and stack size idx", stack_size_idx)
        ...
        SMOOTHING = 200
        accum_score = 0.0
        for training_iter_id in tqdm(range(self._args.n_iterations)):
            for p in range(self._eval_env_bldr.N_SEATS):
                self._ray.wait([
            ...
            scores_all_las = self._ray.get([
                ...
                for la in self._la_handles_0 + self._la_handles_1
            ]) # "self._args.play_n_steps_per_iter_per_la" refers to "# of episodes"
    ...
    def _compute_rlbr(self, stack_size):
        ...
        outcomes = self._ray.get([
            ...
            for la in self._la_handles_0
        ]) # la_0 is for learning the ddqn agent at place 0
        # for la in self._la_handles_0 + self._la_handles_1

        return np.array([x for la_outcomes in outcomes for x in la_outcomes]) # multiple hands at both places
