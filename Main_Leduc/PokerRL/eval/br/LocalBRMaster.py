...
class LocalBRMaster(EvaluatorMasterBase):
        ...
        for mode in self._t_prof.eval_modes_of_algo:
            ...
            for stack_size_idx, stack_size in enumerate(self._t_prof.eval_stack_sizes):
                ...
                if self._eval_agent.can_compute_mode():
                    expl = self._compute_br_heads_up(stack_size_idx=stack_size_idx, iter_nr=iter_nr, is_hier=is_hier)
    ...
    def _compute_br_heads_up(self, stack_size_idx, iter_nr=None, do_export_tree=True, is_hier=False):
        ...
        gt.fill_with_agent_policy(agent=self._eval_agent, is_hier=is_hier) # danger
        gt.compute_ev()
        ...
        # mean over players
        return sum(gt.root.exploitability) * self._env_bldr.env_cls.EV_NORMALIZER / self._t_prof.n_seats
