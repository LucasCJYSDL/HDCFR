...
class IterationStrategy:
    ...
    def get_action(self, pub_obses, range_idxs, legal_actions_lists):
        a_probs = self.get_a_probs(pub_obses=pub_obses, range_idxs=range_idxs,
                                   legal_actions_lists=legal_actions_lists, to_np=False)
        # print("1: ", a_probs, a_probs.shape)
        # print("2: ", torch.multinomial(a_probs, num_samples=1), torch.multinomial(a_probs, num_samples=1).shape)
        return torch.multinomial(a_probs, num_samples=1).cpu().numpy()
    ...
    def get_a_probs2(self, pub_obses, range_idxs, legal_action_masks, to_np=True):
        ...
        with torch.no_grad():
            ...
            if self._iteration == 0:  # at iteration 0
                ...
                # print("1: ", best_legal_deterministic.shape, torch.where(legal_action_masks.byte(), advantages, torch.full_like(advantages, fill_value=-10e20)).shape)
                # print("2: ", torch.where(legal_action_masks.byte(), advantages, torch.full_like(advantages, fill_value=-10e20)))
                # print("3: ", bests, bests.shape)
                _batch_arranged = torch.arange(bs, device=self._device, dtype=torch.long)
                best_legal_deterministic[_batch_arranged, bests] = 1
                # print("4: ", best_legal_deterministic, best_legal_deterministic.shape)
                # """"""""""""""""""""
                # Strat
                # """"""""""""""""""""
                ...
                # )
                #
                # print("5: ", strategy, strategy.shape, uniform_even_legal.shape, uniform_even_legal)
                if to_np:
