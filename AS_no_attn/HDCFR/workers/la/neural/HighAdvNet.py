...
class HighAdvVet(nn.Module):
    def __init__(self, env_bldr, args, device):
        ...
        self._n_pre_options = args.mpm_args.dim_c + 1
        ...
        self._adv = nn.Linear(in_features=args.n_units_final, out_features=self._n_options * self._n_pre_options)
        ...
        self._v = nn.Linear(in_features=args.n_units_final, out_features=self._n_pre_options)
    ...
    def forward(self, pub_obses, range_idxs, option_idxs):
        ...
        adv = self._get_adv(shared_out=shared_out, option_idxs=option_idxs)

        val = F.relu(self._state_v_layer(shared_out))
        val = self._v(val)
        # print("2: ", adv.shape, val.shape)
        val = val.view(-1, self._n_pre_options, 1)
    ...
    def get_adv(self, pub_obses, range_idxs, option_idxs):
        ...
        return self._get_adv(shared_out=shared_out, option_idxs=option_idxs)

    def _get_adv(self, shared_out, option_idxs): # all options are legal
        y = F.relu(self._adv_layer(shared_out))
        y = self._adv(y)

        y = y.view(-1, self._n_pre_options, self._n_options)
        ind = option_idxs.unsqueeze(-1).view(-1, 1, 1).expand(-1, 1, self._n_options)
        y = y.gather(dim=-2, index=ind).squeeze(dim=-2)  # (bs, dim_c)
