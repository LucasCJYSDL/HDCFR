...
class Local_RLBR_LearnerActor(WorkerBase):
    ...
    def __init__(self, t_prof, chief_handle, eval_agent_cls):
        ...
        self._ddqns = [None for _ in range(self._eval_env_bldr.N_SEATS)] # an agent for each seat
    ...
    def play(self, n_episodes):
        ...
        for n in range(n_episodes):
            ...
            # print("1: ", self._opponent.last_options)
            range_idxs = [
            ...
            while not done:
                ...
                # if self._t_prof.DEBUGGING: # sync the two environments
            ...
            # Add terminal memory # different from add_experience, add_to_buf is specific for the terminal obs
    ...
    def compute_rlbr(self, n_hands_each_seat, ddqn_state_dicts, stack_size):
        ...
        for rlbr_seat_id in range(rlbr_env_wrapper.env.N_SEATS):
            ...
            for iteration_id in tqdm(range(n_hands_each_seat)):
                ...
                # Play Episode
                # """""""""""""""""
                while not done: # how to set who play first
