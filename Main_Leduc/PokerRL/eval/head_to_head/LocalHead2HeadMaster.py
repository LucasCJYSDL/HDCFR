...
class LocalHead2HeadMaster(EvaluatorMasterBase):
    ...
    def evaluate(self, iter_nr):
        ...
        for stack_size_idx, stack_size in enumerate(self._t_prof.eval_stack_sizes):
            ...
            if do_it:
                winnings = self._run_eval(stack_size=stack_size)
        ...
        #         print("The head-to-head result is with mean {} and 95 confidence interval {}.".format(mean, d))
        #
        #     if self._is_multi_stack:
    ...
    def _run_eval(self, stack_size):
        winnings = np.empty(shape=(int(self._args.n_hands * self._env_bldr.N_SEATS), ), dtype=np.float32)
        ...
        for seat_p0 in range(self._env_bldr.N_SEATS):
            ...
            for iteration_id in tqdm(range(self._args.n_hands)):
