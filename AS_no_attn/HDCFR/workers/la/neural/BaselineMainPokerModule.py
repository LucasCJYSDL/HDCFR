...
class BaselineMainPokerModule(nn.Module):
    ...
    def __init__(self,
        ...
        if mpm_args.deep:
            ...
            # TODO: include option history as well
            ...
            self.layer_1 = nn.Linear(in_features=self.card_emb.out_size * 2 + self._env_bldr.pub_obs_size
    ...
    # TODO: get rid of the option emb
    def forward(self, pub_obses, range_idxs):
        ...
        card_o_0 = self.card_emb(pub_obses=pub_obses,
                                 range_idxs=torch.where(range_idxs_0 == 8888, torch.zeros_like(range_idxs_0), range_idxs_0))
        card_o_0 = torch.where(range_idxs_0.unsqueeze(1).expand_as(card_o_0) == 8888,
                               torch.full_like(card_o_0, fill_value=-1), card_o_0)

        card_o_1 = self.card_emb(pub_obses=pub_obses,
                                 range_idxs=torch.where(range_idxs_1 == 8888, torch.zeros_like(range_idxs_1), range_idxs_1))
        card_o_1 = torch.where(range_idxs_1.unsqueeze(1).expand_as(card_o_1) == 8888,
                               torch.full_like(card_o_1, fill_value=-1), card_o_1)
        card_o = torch.cat([card_o_0, card_o_1], dim=-1)

        # option embeddings
        # assert option_emb
        # ct = option_idxs.to(self._device)  # should be torch long, (bs, )
        # ct_o = option_emb(ct.unsqueeze(0)).detach().squeeze(0)  # (bs, dim_e)
...
class BaselineMPMArgs:
    ...
    def get_mpm_cls(self):
        return BaselineMainPokerModule
