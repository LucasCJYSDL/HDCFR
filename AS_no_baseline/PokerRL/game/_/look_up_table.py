...
class _LutHolderBase:
    ...
    def __init__(self, lut_getter):
        ...
        self.LUT_RANGE_IDX_TO_PRIVATE_OBS = self._lut_getter.get_range_idx_to_private_obs_LUT() # private observation
    ...
    # TODO test
    def get_range_idxs_blocked_by_range_idx(self, range_idx):
        return np.unique(np.concatenate(
            tuple([
                self.LUT_CARD_IN_WHAT_RANGE_IDXS[c]
                for c in self.get_1d_hole_cards_from_range_idx(range_idx)
            ])
        ))
