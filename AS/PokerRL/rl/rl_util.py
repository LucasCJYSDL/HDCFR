...
def str_to_loss_cls(loss_str):
    ...
    elif loss_str.lower() == "hdcfr_loss":
        return lambda y, trgt, w: torch.mean(torch.sum((y - trgt) ** 2, dim=-1))

    elif loss_str.lower() == "hdcfr_baseline_loss":
        return lambda y, trgt, w: w * torch.sum((y - trgt)**2, dim=-1)
...
def get_legal_action_mask_torch(n_actions, legal_actions_list, device, dtype=torch.uint8):
    ...
    """
    idxs = torch.LongTensor(legal_actions_list)
    mask = torch.zeros((n_actions), dtype=dtype)
    mask[idxs] = 1
...
def batch_get_legal_action_mask_torch(n_actions, legal_actions_lists, device, dtype=torch.uint8):
    ...
    mask = torch.zeros((len(legal_actions_lists), n_actions,), dtype=dtype)
    # print(mask.device)
    for i, legal_action_list in enumerate(legal_actions_lists):
        mask[i, torch.LongTensor(legal_action_list)] = 1
    # print(mask.device, mask.to(device=device).device)
