...
class DuelingQNet(nn.Module):
    ...
    def forward(self, pub_obses, range_idxs, legal_action_masks):
        ...
        val = F.relu(self._state_v_layer(shared_out)) # weird design of the regret function
        val = self._v(val).expand_as(adv)
        # print("2: ", adv.shape, val.shape)
        return (val + adv) * legal_action_masks # it's more like return function rather than an advantage function
