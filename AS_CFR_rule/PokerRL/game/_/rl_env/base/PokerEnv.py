...
class PokerEnv:
    ...
    # TODO: test the upper limit of these variables
    def _construct_obs_space(self): # definition of the observation space
        """
    ...
    def _step(self, processed_action):
        ...
        if processed_action[0] == Poker.CHECK_CALL:
            ...
            self.n_actions_this_episode += 1 # why actions only include raise
    ...
    def _get_new_board(self): # -127 represent not dealt cards
    ...
    def _get_fixed_action(self, action): # a converter of output actions, tricky
        """
    ...
    def _get_board_state(self, ):
        _board_space = [Poker.CARD_NOT_DEALT_TOKEN_1D for _ in range(self.N_TOTAL_BOARD_CARDS * 3)]
        for i, card in enumerate(self.board.tolist()):
            if card[0] == Poker.CARD_NOT_DEALT_TOKEN_1D:
                break

            D = 3 * i
            _board_space[D] = card[0]

            # only include suit in obs if it is relevant to the game rules played
            if self.SUITS_MATTER:
                _board_space[D + 1] = card[1]
                _board_space[D + 2] = self.lut_holder.get_1d_card(card_2d=card)
    ...
    def _get_step_reward(self, is_terminal):
        if not is_terminal:
            return np.zeros(shape=self.N_SEATS, dtype=np.float32).tolist()
    ...
    def state_dict(self):
        env_state_dict = {
            ...
            EnvDictIdxs.side_pots: copy.copy(self.side_pots),  # list
            EnvDictIdxs.main_pot: self.main_pot,  # int by value
            EnvDictIdxs.board_2d: np.copy(self.board),  # np array
            EnvDictIdxs.last_action: copy.copy(self.last_action),
    ...
    def load_state_dict(self, env_state_dict, blank_private_info=False):
        ...
        self.side_pots = copy.copy(env_state_dict[EnvDictIdxs.side_pots])
        self.main_pot = env_state_dict[EnvDictIdxs.main_pot]
        self.board = np.copy(env_state_dict[EnvDictIdxs.board_2d])
        self.last_action = copy.copy(env_state_dict[EnvDictIdxs.last_action])
        ...
        e = env_state_dict[EnvDictIdxs.seats]
        for p in self.seats:
            p.load_state_dict(state=e[p.seat_id], blank_private_info=blank_private_info)
    ...
    def get_current_obs(self, is_terminal):
        ...
        return np.array(self._get_table_state(normalization_sum=self.REWARD_SCALAR) \
                        + self._get_player_states_all_players(normalization_sum=self.REWARD_SCALAR) \
    ...
    def print_obs(self, obs):
        ...
        names = [str(i) + " " + e + ":  " for i, e in enumerate(list(self.obs_idx_dict.keys()))]
    ...
    def get_max_raise_frac(self, raise_legal=None):
        if raise_legal is None:
            a = self._get_env_adjusted_action_formulation(action=(Poker.BET_RAISE, 1,))
            fixed_a = self._get_fixed_action(action=a)
            raise_legal = a[0] == fixed_a[0]

        if raise_legal:
            return self.get_frac_from_chip_amt(amt=self.current_player.current_bet + self.current_player.stack,
    ...
    def get_range_idx(self, p_id):
        ...
        """
        return int(self.lut_holder.get_range_idx_from_hole_cards(hole_cards_2d=self.seats[p_id].hand))
