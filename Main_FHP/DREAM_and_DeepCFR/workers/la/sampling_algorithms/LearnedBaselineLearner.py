...
class BaselineWrapper(NetWrapperBase):
    ...
    def __init__(self, env_bldr, baseline_args, device):
        super().__init__(
            net=DuelingQNet(env_bldr=env_bldr, q_args=baseline_args.q_net_args, device=device),
            owner=None,
            env_bldr=env_bldr,
            args=baseline_args,
            device=device,
    ...
    def _mini_batch_loop(self, buffer, grad_mngr):
        ...
        # this loss function design seems totally irrelevant to the design from VR-MCCFR
        grad_mngr.backprop(pred=q1_t_of_a_selected, target=target) # the loss function is mse
