...
class ReservoirBufferBase:
    ...
    def __init__(self, owner, max_size, env_bldr, nn_type, iter_weighting_exponent):
        ...
        self.device = torch.device("cpu") # unlike the networks, the buffer should be stored in cpu
        self.size = 0
        ...
        self._last_option_buffer = torch.zeros((max_size,), dtype=torch.long, device=self.device)
        self._cur_option_buffer = torch.zeros((max_size,), dtype=torch.long, device=self.device)
        self._legal_action_mask_buffer = torch.zeros((max_size, env_bldr.N_ACTIONS,),
                                                     dtype=torch.float32, device=self.device)
        self._high_iteration_buffer = torch.zeros((max_size,), dtype=torch.float32, device=self.device)
        self._low_iteration_buffer = torch.zeros((max_size,), dtype=torch.float32, device=self.device)
        self._iter_weighting_exponent = iter_weighting_exponent

        self._last_high_iteration_seen = None
        self._last_low_iteration_seen = None
    ...
    def state_dict(self):
        return {
            ...
            "last_option_buffer": self._last_option_buffer,
            "cur_option_buffer": self._cur_option_buffer,
            "legal_action_mask_buffer": self._legal_action_mask_buffer,
            "high_iteration_buffer": self._high_iteration_buffer,
            "low_iteration_buffer": self._low_iteration_buffer,
        }
    ...
    def load_state_dict(self, state):
        ...
        if self._nn_type == "recurrent":
            ...
            self._last_option_buffer = state["last_option_buffer"]
            self._cur_option_buffer = state["cur_option_buffer"]
            self._legal_action_mask_buffer = state["legal_action_mask_buffer"]
            # self._iteration_buffer = state["iteration_buffer"]
            self._high_iteration_buffer = state["high_iteration_buffer"]
            self._low_iteration_buffer = state["low_iteration_buffer"]
            ...
            self._last_option_buffer = state["last_option_buffer"].to(self.device)
            self._cur_option_buffer = state["cur_option_buffer"].to(self.device)
            self._legal_action_mask_buffer = state["legal_action_mask_buffer"].to(self.device)
            self._high_iteration_buffer = state["high_iteration_buffer"].to(self.device)
            self._low_iteration_buffer = state["low_iteration_buffer"].to(self.device)
