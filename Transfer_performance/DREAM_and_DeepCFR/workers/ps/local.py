...
class ParameterServer(ParameterServerBase):
    ...
    def __init__(self, t_prof, owner, chief_handle):
        ...
        if self._BASELINE and owner == 0: # only main baseline for player 0
    ...
    def checkpoint(self, curr_step):
        ...
        if self._BASELINE and self.owner == 0:
    ...
    def _get_new_baseline_net(self):
        return DuelingQNet(q_args=self._baseline_args.q_net_args, env_bldr=self._env_bldr, device=self._device)
