...
class FlatLimitPokerEnvBuilder(_EnvWrapperBuilderBase):
    ...
    def __init__(self, env_cls, env_args):
        ...
        N = 2  # bet/raise  +   check/call
        for r in env_cls.RULES.ALL_ROUNDS_LIST: # history of actions, which is part of the state
            self._VEC_ROUND_OFFSETS[r] = self.action_vector_size
            # self._VEC_HALF_ROUND_SIZE[r] = N * (env_cls.MAX_N_RAISES_PER_ROUND[r] + 2)
            self._VEC_HALF_ROUND_SIZE[r] = N * (20 + 2)
        ...
        # print("here:", _env.observation_space.shape[0], self.action_vector_size)
        # 25, 32 (2*4*2 + 2*4*2)
