...
class LearnedBaselineSampler(SamplerBase):
    ...
    def _traverser_act(self, start_state_dict, traverser, trav_depth, plyrs_range_idxs, iteration_strats, sample_reach,
        ...
        return (utility.to(strat_i.device) * strat_i).sum(), strat_i
    ...
    def _any_non_traverser_act(self, start_state_dict, traverser, plyrs_range_idxs, trav_depth, iteration_strats,
        ...
        current_pub_obs = self._env_wrapper.get_current_obs() # current env obs + obs history
        ...
        # """""""""""""""""""""""""
        # a main spot that needs to be changed
        ...
        # print("3: ", plyrs_range_idxs, _crazy_embed(plyrs_range_idxs=plyrs_range_idxs))
        # [2, 4], 20004
        ...
        self._baseline_buf.add(
            ...
            range_idx_crazy_embedded=_crazy_embed(plyrs_range_idxs=plyrs_range_idxs),
            legal_action_mask=legal_action_mask,
            r=rew_for_all[0],  # 0 bc we mirror for 1... zero-sum #??? TODO: not aligh with the paper
            a=a,
            done=done,
        ...
        return (utility.to(strat_opp.device) * strat_opp).sum(), strat_opp
    ...
    def _get_utility(self, traverser, pub_obs, range_idx_crazy_embedded,
        ...
        # print(legal_action_mask.device)
        utility = baselines.to(legal_action_mask.device) * legal_action_mask
        # print("2: ", utility, utility.shape)
        utility[a] += (u_bootstrap - utility[a]) / sample_strat[a]
        # print("1: ", baselines, baselines.shape, utility, utility.shape, u_bootstrap)

        return utility


# See MPM_Baseline, an embedding that contains the hole cards from both players, probably is not a good embedding
