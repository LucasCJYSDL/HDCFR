...
class HighLevelAlgo(_HighLevelAlgoBase):
    ...
    def run_one_iter_alternating_update(self, cfr_iter):
        ...
        if self._BASELINE:
            t_computation_baseline, t_syncing_baseline = \
                self._train_baseline(n_updates=self._baseline_args.n_batches_per_iter_baseline)
    ...
    def _generate_traversals(self, p_id, cfr_iter):
        ...
        if p_id == 1:
            self._ray.wait([
                self._ray.remote(self._chief_handle.add_scalar,
                                 self._exp_states_traversed, "States Seen", cfr_iter,
                                 sum(states_seen)
                                 )
            ])
    ...
    def _update_leaner_actors(self, update_adv_for_plyrs=None, update_avrg_for_plyrs=None,
        ...
        w_baseline = [None if not update_baseline else self._ray.remote(
            self._ps_handles[0].get_baseline_weights)]
    ...
    def _train_baseline(self, n_updates):
        ...
        for epoch_nr in tqdm(range(n_updates)):
            ...
            t0 = time.time()
            self._ray.wait([
                self._ray.remote(self._ps_handles[0].apply_grads_baseline,
                                 grads_from_all_las)
            ])
