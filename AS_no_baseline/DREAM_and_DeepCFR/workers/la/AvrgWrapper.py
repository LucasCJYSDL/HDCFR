...
class AvrgWrapper(_NetWrapperBase):
    ...
    def _mini_batch_loop(self, buffer, grad_mngr): # main component, specific for each network agent
        ...
        batch_a_probs, \
        batch_loss_weight, \
            = buffer.sample(device=self.device, batch_size=self._args.batch_size) # the a_prob must be the action distribution, or it would be a huge bug
        # print("1: ", batch_a_probs)
