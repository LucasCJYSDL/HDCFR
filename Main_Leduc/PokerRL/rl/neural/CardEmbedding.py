...
class CardEmbedding(nn.Module):
    ...
    def forward(self, pub_obses, range_idxs):
        priv_cards_1d = self._lut_range_idx_to_1d[range_idxs.cpu()]
        # print(priv_cards_1d, range_idxs)
        ...
        card_o = []
        # print("1: ", len(self.card_embs), len(card_batches), card_batches)
        for emb, (ranks, suits, cards) in zip(self.card_embs, card_batches):
            card_o.append(emb(ranks=ranks, suits=suits, cards=cards))
        # print("2: ", card_o)
        # print("3: ", torch.cat(card_o, dim=1).shape)
        return torch.cat(card_o, dim=1) # cat over private cards, and public cards in each round (bs, n_cards_type * emb_dim)
...
class _CardGroupEmb(nn.Module):
    ...
    def __init__(self, env_bldr, dim):
        ...
        self._env_bldr = env_bldr
        self._dim = dim
        self.rank = nn.Embedding(env_bldr.rules.N_RANKS, dim) # N_RANKS of tensors of size dim
    ...
    def forward(self, ranks, suits, cards):
        bs, n_cards = cards.shape # n_cards refer to the number of hole/revealed board cards
        # print("0: ", cards, ranks, suits)
        # print("1: ", cards.shape, ranks.shape, suits.shape)
        r = ranks.view(-1)
        valid_r = r.ge(0).unsqueeze(1).to(torch.float32)
        r = r.clamp(min=0)
        embs = self.rank(r) * valid_r
        # print("2: ", r.shape, embs.shape)
        ...
        # print("3: ", embs.shape, embs.view(bs, n_cards, -1).sum(1).shape)
        return embs.view(bs, n_cards, -1).sum(1) # sum over multiple hole/revealed board cards
